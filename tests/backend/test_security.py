...
class TestEnvironmentVariableHandling:
    ...
    def test_get_required_env_var_missing(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ConfigurationError, match="TEST_VAR environment variable is required"):
...
class TestSecureLogger:
    ...
    def test_secure_logger_all_levels(self):
        ...
        for method in [mock_logger.debug, mock_logger.info, mock_logger.warning, 
                      mock_logger.error, mock_logger.exception]:
...
class TestDeepgramSTTSecurity:
    ...
    def test_deepgram_custom_env_var(self):
        ...
        with patch.dict(os.environ, {"DEEPGRAM_API_KEY": "key123"}, clear=True):
            with pytest.raises(DeepgramSTTError, match="Deepgram API key not found: CUSTOM_DEEPGRAM_KEY"):
                DeepgramSTT(stt_config, utterance_callback)
