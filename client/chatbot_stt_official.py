#!/usr/bin/env python3
"""
Working Deepgram SDK STT integration for ChatBot.
...
class ChatBotSTTOfficial:
    """
    Official Deepgram SDK-style STT integration.
    ...
    def __init__(self, stt_config: dict, utterance_callback: Callable[[str], None]):
        ...
        # Create a dedicated event loop for Deepgram tasks (like working testdeepgram.py)
    ...
    def _run_dg_loop(self):
        """Run the dedicated asyncio event loop for Deepgram operations (like working testdeepgram.py)."""
        import threading
    ...
    # Sync wrapper methods using dedicated event loop (like working testdeepgram.py)
    def start(self):
